...
import { Plus, Loader2, PackagePlus, CalendarDays } from "lucide-react";
...
interface Transaction { id: string; productId: string; type: string; quantity: number; note: string | null; size: string | null; createdByName: string | null; createdAt: string; product: Product }
...
export default function StockInPage() {
  ...
  async function fetchRecap() {
    setRecap(await fetch("/api/stock-transactions/recap?type=in").then((r) => r.json()));
  }
      ...
      if (selectedProduct?.isClothing) {
        ...
        for (const [size, quantity] of entries) {
          const res = await fetch("/api/stock-transactions", {
            ...
            body: JSON.stringify({ productId: form.productId, type: "in", quantity, size, note: form.note || null }),
          });
        ...
        const res = await fetch("/api/stock-transactions", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ productId: form.productId, type: "in", quantity: form.quantity, size: null, note: form.note || null }),
        });
        if (!res.ok) { const err = await res.json(); toast.error(err.error || "Gagal menyimpan"); setLoading(false); return }
      }
      toast.success("Barang masuk berhasil dicatat");
      setOpen(false); setForm({ productId: "", quantity: 1, note: "", size: "" }); setSizeQtys({});
      ...
      {tab === "list" ? (
        ...
        <>
          <div className="grid grid-cols-3 gap-3">
            ...
            <Card className="p-4 text-center">
              <p className="text-sm text-muted-foreground">Total Item Masuk</p>
              <p className="text-2xl font-bold text-green-600">{recap.reduce((s, r) => s + r.totalQty, 0)}</p>
            </Card>
            <Card className="p-4 text-center">
            ...
            {fetchLoading ? <TableSkeleton columns={4} /> : <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Item Masuk</TableHead>
              ...
              <TableBody>
                {recap.map((r) => (
                  <TableRow key={r.date}>
                    ...
                    <TableCell className="text-right font-semibold text-green-600">+{r.totalQty}</TableCell>
